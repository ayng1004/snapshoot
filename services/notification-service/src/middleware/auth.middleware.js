...
// Special middleware for service-to-service communication
const serviceAuthMiddleware = (req, res, next) => {
  try {
    // Vérifier si le header Authorization existe
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('ServiceToken ')) {
      return res.status(401).json({ message: 'Accès non autorisé - Service Token manquant' });
    }
    ...
    if (token !== process.env.SERVICE_TOKEN) {
      return res.status(401).json({ message: 'Accès non autorisé - Service Token invalide' });
    }

    next();
  } catch (error) {
    logger.error(`Erreur d'authentification de service: ${error.message}`);
    return res.status(401).json({ message: 'Accès non autorisé' });
  }
};
